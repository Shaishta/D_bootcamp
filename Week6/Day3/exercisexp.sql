...
-- #2(1)
-- select film.title, film.description, language.name from film full outer join language on language.language_id = film.language_id;
-- #2(2)
...
-- insert into new_film (name) values ('Titanic'),('The Gentlemen'), ('The Lion King');

-- #4
-- create table customer_review (
-- review_id serial primary key not null, 
-- film_id smallint references new_film(id) on delete cascade, 
-- language_id smallint references language(language_id) on delete cascade, 
...
-- #5
-- insert into customer_review (film_id, language_id, title, score, review_text, last_update) values (3, 1, 'The Lion King review', 8, 'Extremely good movie!!!','2021/12/01');
-- insert into customer_review (film_id, language_id, title, score, review_text, last_update) values (1, 1, 'Titanic review', 8, 'Very emotional and romantic movie','2021/12/01');
-- select * from customer_review;
-- #6
-- delete from new_film where name = 'The Lion King';

-- Exercise 2 : DVD Rental
-- #1
-- select name from language;
-- update film set language_id = 4 where title like'B%';
-- #2
-- Foreign-key constraints: "customer_address_id_fkey" 
-- FOREIGN KEY (address_id) 
-- REFERENCES address(address_id) ON UPDATE CASCADE ON DELETE RESTRICT 
-- Referenced by: TABLE "payment" CONSTRAINT "payment_customer_id_fkey" FOREIGN KEY (customer_id) REFERENCES customer(customer_id) ON UPDATE CASCADE ON DELETE RESTRICT TABLE "rental" CONSTRAINT "rental_customer_id_fkey" FOREIGN KEY (customer_id) REFERENCES customer(customer_id) ON UPDATE CASCADE ON DELETE RESTRICT
-- when we insert into the costumer table it changes(updates) the payment and rental tables
-- #3
-- drop table customer_review;
-- it is an easy step, the table does not affect other tables.
-- #4
...
-- #6
-- #6(1)
-- select first_name, last_name, actor.actor_id, title, film.description from film_actor inner join actor on film_actor.actor_id = actor.actor_id inner join film on film.film_id = film_actor.film_id where first_name = 'Penelope' and last_name = 'Monroe' and description ilike  '%sumo wrestler%';
-- #6(2)
